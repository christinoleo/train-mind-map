...
export class OverlayManager {
  ...
  /** Call once a frame: redraws the enabled overlays if the map changed. */
  refresh(): void {
    const { map } = this.state;
    if (this.drawnMap === map && this.drawnRing === map.revealedRing) return;
    this.drawnMap = map;
    this.drawnRing = map.revealedRing;
    for (const entry of this.entries.values()) {
      if (entry.enabled) this.redraw(entry);
    }
  }
