...
const CODECS: Readonly<Record<string, Codec>> = {
  ...
  CreateLine: codec(CreateLine, (c) => [c.stops]),
  PlaceTrain: codec(PlaceTrain, (c) => [c.line]),
  RemoveTrain: codec(RemoveTrain, (c) => [c.id]),
  RemoveLine: codec(RemoveLine, (c) => [c.id]),
  SetDeparture: codec(SetDeparture, (c) => [c.line, c.stop, c.condition]),
